...
subtest 'empty where, opt' => sub {
    my ( $relationship, $src_table, $src_columns, $where, $opt ) = $r->fetch_src();
    is $relationship, $r;
    is $src_table,    'friend';
...
subtest 'where' => sub {
    my ( $relationship, $src_table, $src_columns, $where, $opt ) = $r->fetch_src( { user_id => 1 } );
    is $relationship, $r;
    is $src_table,    'friend';
    is_deeply $src_columns, ['user_id'];
